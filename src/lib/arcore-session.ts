...
let xrRefSpace:   any = null  // XRReferenceSpace — typed as any for TS compat
...
export async function startARSession(overlayElement: HTMLElement): Promise<boolean> {
  ...
  try {
    ...
    xrRefSpace = await (xrSession as any).requestReferenceSpace('local-floor')  // eslint-disable-line
