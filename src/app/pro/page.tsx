...
export default function ProPage() {
  return (
    <main style={{ minHeight: '100dvh', background: C.dark, color: '#fff', fontFamily: "system-ui, -apple-system, 'Segoe UI', sans-serif" }}>{/* Nav */}
      <nav style={{ padding: '1.2rem 1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid rgba(255,255,255,0.06)' }}><a href="/" style={{ fontSize: '0.55rem', letterSpacing: '0.04em', color: C.orange, textDecoration: 'none' }}>{/* eslint-disable-next-line @next/next/no-img-element */}
      ...
      {/* Hero */}
      <section style={{ maxWidth: 480, margin: '0 auto', padding: '3rem 1.5rem 1.5rem', textAlign: 'center' }}><div style={{ display: 'inline-block', background: 'rgba(0,127,255,0.15)', border: '1px solid rgba(0,127,255,0.3)', borderRadius: 20, padding: '0.3rem 1rem', fontSize: '0.62rem', letterSpacing: '0.04em', color: C.blue, marginBottom: '1rem' }}>PRO PLAN
        </div>
        <h1 style={{ fontSize: '2.2rem', fontWeight: 700, letterSpacing: '-0.03em', lineHeight: 1.1, margin: '0 0 0.75rem' }}>AR-accurate stair<br />measurements
        </h1>
        <p style={{ fontSize: '0.88rem', color: 'rgba(255,255,255,0.5)', lineHeight: 1.7, margin: '0 0 2rem' }}>Upgrade from AI estimation to true plane-detection AR. Sub-5mm accuracy on ARCore and ARKit devices.
        </p>
        <div style={{ fontSize: '2.4rem', fontWeight: 700, color: '#fff', marginBottom: '0.2rem' }}>$38.99<span style={{ fontSize: '1rem', fontWeight: 400, color: 'rgba(255,255,255,0.4)' }}>/month</span>
        </div>
      ...
      <section style={{ textAlign: 'center', padding: '2rem 1.5rem 4rem' }}><a href="https://calendly.com/staircode/30min" target="_blank" rel="noopener noreferrer" style={{
          ...
          background: `linear-gradient(135deg,${C.blue},${C.orange})`,
          borderRadius: 16, color: '#fff', textDecoration: 'none',
          fontSize: '0.88rem', fontWeight: 700, letterSpacing: '0.1em', boxShadow: '0 4px 24px rgba(0,127,255,0.35)',
        }}>Book a Call →
        </a>
