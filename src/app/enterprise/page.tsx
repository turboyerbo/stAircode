...
export default function EnterprisePage() {
  return (
    <main style={{ minHeight: '100dvh', background: C.dark, color: '#fff', fontFamily: "system-ui, -apple-system, 'Segoe UI', sans-serif" }}>
      ...
      <section style={{ maxWidth: 520, margin: '0 auto', padding: '3rem 1.5rem 1.5rem', textAlign: 'center' }}>
        <div style={{ display: 'inline-block', background: 'rgba(255,127,0,0.15)', border: '1px solid rgba(255,127,0,0.3)', borderRadius: 20, padding: '0.3rem 1rem', fontSize: '0.62rem', fontFamily: 'monospace', letterSpacing: '0.14em', color: C.orange, marginBottom: '1rem' }}>
          ENTERPRISE
        </div>
        <h1 style={{ fontSize: '2rem', fontWeight: 900, letterSpacing: '-0.03em', lineHeight: 1.1, margin: '0 0 0.75rem' }}>
          Built for architecture<br />firms and studios
        </h1>
        <p style={{ fontSize: '0.88rem', color: 'rgba(255,255,255,0.5)', lineHeight: 1.7, margin: '0 0 2rem' }}>
          Team-wide access, API integration with your workflow, custom code libraries, and white-label PDF reports.
        </p>
        <div style={{ fontSize: '2.4rem', fontWeight: 900, color: '#fff', marginBottom: '0.2rem' }}>
          $79<span style={{ fontSize: '1rem', fontWeight: 400, color: 'rgba(255,255,255,0.4)' }}>/month</span>
        </div>
      ...
      <section style={{ maxWidth: 520, margin: '0 auto', padding: '2rem 1.5rem' }}>
        {[
          ...
          { icon: '🏷️', title: 'White-Label Reports', desc: 'PDF reports branded with your firm logo and contact details.' },
        ].map(f => (
