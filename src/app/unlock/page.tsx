    ...
    <div style={{ minHeight: '100dvh', background: C.bg, fontFamily: "-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif", color: C.text }}>
      ...
      <div style={{ maxWidth: 520, margin: '0 auto', padding: '2rem 1.25rem 4rem' }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '1.4rem', marginBottom: '0.3rem' }}>{overall ? '✅' : '⚠️'}</div>
          <div style={{ fontSize: '1.1rem', fontWeight: 900, color: overall ? C.green : C.red, marginBottom: '0.25rem' }}>
            {fields.length === 0 ? 'Scan results' : overall ? 'No issues detected' : `${failed.length} item${failed.length > 1 ? 's' : ''} flagged`}
          </div>
