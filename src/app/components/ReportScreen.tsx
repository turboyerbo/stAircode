 ...
 *
 * The Included with subscription report is generated by Claude via /api/vision with a text-only
...
export default function ReportScreen({ measurements, fields, codeLabel, codeRef, location, userLatLng, isOntario, userRole, onRetake, onStartOver }: Props) {
  ...
  // Step 3: Paywall — "Get Full Report Included with subscription" → Stripe → download
        ...
        <>
          ...
          <div style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 430, zIndex: 100, background: profile.bg3, borderRadius: '22px 22px 0 0', padding: sheet === 'blurred-preview' ? '0' : '1.25rem 1.25rem 3rem', display: 'flex', flexDirection: 'column', gap: '0.9rem', boxShadow: '0 -8px 40px rgba(0,0,0,0.7)', maxHeight: '90dvh', overflowY: sheet === 'blurred-preview' ? 'hidden' : 'auto' }}>{sheet !== 'blurred-preview' && <div style={{ width: 36, height: 4, borderRadius: 2, background: 'rgba(147,186,212,0.25)', alignSelf: 'center', marginBottom: '0.1rem', flexShrink: 0 }} />}
              ...
              return (
                <div style={{ position: 'relative', borderRadius: '22px 22px 0 0', overflow: 'hidden', background: '#0A1C2E' }}>{/* ── Drag pill ─────────────────────────────────────────── */}
                  ...
                  <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'linear-gradient(to top, rgba(6,18,32,0.98) 45%, rgba(6,18,32,0.7) 75%, transparent 100%)', padding: '2.5rem 1.25rem 2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.75rem' }}><div style={{ textAlign: 'center' }}><div style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.5)', marginBottom: '0.3rem' }}>Your full report is ready</div>
                      <div style={{ fontSize: '1.05rem', fontWeight: 700, color: '#fff', letterSpacing: '-0.02em', lineHeight: 1.25 }}>Unlock with Subscription
                      </div>
                    ...
                    <button
                      ...
                      style={{ width: '100%', padding: '1rem 1.5rem', background: `linear-gradient(135deg, ${GOLD}, #C4721E)`, border: 'none', borderRadius: 16, color: '#000', fontSize: '1rem', fontWeight: 700, letterSpacing: '0.05em', cursor: 'pointer', boxShadow: `0 4px 24px rgba(242,147,55,0.45)`, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>Unlock Full Report — Included with subscription →
                    </button>
            ...
            {sheet === 'plans' && <>
              ...
              {/* ── Included with subscription REPORT ── */}
                ...
                {isTestVariant ? (
                  <div>
                    ...
                    {trialExhausted ? (
                      ...
                      <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:'0.4rem', marginTop:'0.45rem' }}><span style={{ fontSize:'0.65rem', color:'#F29337', fontWeight:700 }}>Included with subscription beta</span>
