...
export default function HelpScreen() {
  ...
  return (
    <div style={{ flex:1, overflowY:'auto', background:BG, backgroundImage:'repeating-linear-gradient(0deg,transparent,transparent 27px,rgba(65,124,164,0.07) 27px,rgba(65,124,164,0.07) 28px),repeating-linear-gradient(90deg,transparent,transparent 27px,rgba(65,124,164,0.07) 27px,rgba(65,124,164,0.07) 28px)', color:TEXT, fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif", paddingBottom:'2rem' }}>{/* HEADER */}
      ...
      <div style={{ margin:'0 1rem 1rem', borderRadius:18, overflow:'hidden', border:`1.5px solid ${ms.color}44` }}>{/* Step nav bar — large prominent arrows, no scrollbar */}
        <div style={{ background: `${ms.color}12`, borderBottom:`1px solid ${ms.color}22`, padding:'0.7rem 1rem', display:'flex', alignItems:'center', justifyContent:'space-between', gap:'0.75rem' }}><button onClick={() => setActiveMeasure(i => Math.max(0, i-1))} disabled={activeMeasure===0}
          ...
          <div style={{ display:'flex', gap:'0.5rem', alignItems:'center', flex:1, justifyContent:'center' }}>{MEASURE_STEPS.map((_,i) => (
        ...
        {/* Text */}
        <div style={{ background:BG2, padding:'0.75rem 1.1rem 1rem' }}><div style={{ fontSize:'0.92rem', fontWeight:800, color:TEXT, lineHeight:1.3, marginBottom:'0.5rem' }}>{ms.title}</div>
          <div style={{ fontSize:'0.78rem', color:TEXT2, lineHeight:1.7, marginBottom:'0.65rem' }}>{ms.desc}</div>
          <div style={{ background:`${ms.color}0D`, border:`1px solid ${ms.color}30`, borderRadius:12, padding:'0.6rem 0.85rem', display:'flex', gap:'0.5rem', alignItems:'flex-start' }}><span style={{ fontSize:'0.85rem', flexShrink:0 }}>{ms.icon}</span>
      ...
      <div style={{ margin:'0 1rem 1rem' }}>{/* Intro */}
        <div style={{ background:BG2, border:`1.5px solid ${BORDER}`, borderRadius:14, padding:'0.85rem 1rem', marginBottom:'0.75rem' }}><div style={{ fontSize:'0.78rem', color:TEXT2, lineHeight:1.65 }}>Follow us on Instagram for <strong style={{ color:TEXT }}>step-by-step video guides</strong> showing exactly how to position your phone for each of the 6 scan positions — riser, tread, nosing, handrail, width, and headroom.
          </div>
        </div>
        {/* Instagram card */}
        <a href="https://www.instagram.com/staircode/" target="_blank" rel="noopener noreferrer"
          ...
          <div style={{ flex:1 }}><div style={{ fontSize:'0.92rem', fontWeight:800, color:'#fff', marginBottom:'0.2rem' }}>@staircode.app</div>
            <div style={{ fontSize:'0.72rem', color:'rgba(255,255,255,0.85)', lineHeight:1.5 }}>How-to videos · Scan guides · Tips &amp; updates
            </div>
          </div>
      ...
      <div style={{ margin:'0 1rem' }}>{FAQ.map((item, i) => (
          <div key={i} style={{ background:BG2, border:`1.5px solid ${BORDER}`, borderRadius:14, marginBottom:'0.45rem', overflow:'hidden' }}><button onClick={() => setOpenFAQ(openFAQ===i ? null : i)} style={{ width:'100%', padding:'0.9rem 1rem', background:'transparent', border:'none', cursor:'pointer', textAlign:'left' as const, display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:'0.6rem' }}><span style={{ fontSize:'0.85rem', fontWeight:700, color:TEXT, lineHeight:1.4 }}>{item.q}</span>
              <span style={{ color:TEXT2, fontSize:'1.1rem', flexShrink:0, transition:'transform 0.2s', transform:openFAQ===i?'rotate(45deg)':'none' }}>+</span>
            </button>
            {openFAQ === i && (
              <div style={{ padding:'0 1rem 0.9rem', paddingTop:'0.6rem', fontSize:'0.78rem', color:TEXT2, lineHeight:1.7, borderTop:`1px solid ${BORDER}` }}>{item.a}
              </div>
            )}
          </div>
        ))}
      ...
      <div style={{ margin:'0 1rem 1.5rem', background:BG2, border:`1.5px solid ${BORDER}`, borderRadius:16, padding:'1.2rem', display:'flex', flexDirection:'column', gap:'0.8rem' }}><div>
