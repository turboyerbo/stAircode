...
const MEASURE_STEPS = [
  {
    id: 'detect', image: '/Measure_01_detect.png', badge: '01', label: 'AI Scanning', color: BLUE,
    title: 'You tap Capture — AI analyses the photo instantly',
    desc: 'When you tap the blue Capture button, stAIrcode takes a full-resolution photo. The image is sent to Claude AI Vision which analyses it in seconds. The red beam and dots show the measurement line the AI identifies — from one edge of the dimension to the other.',
    detail: 'The AI uses everything visible in the scene as a scale reference: door frames, skirting boards, floor planks, brick courses, and the phone body itself. The more context in the frame, the more accurate the result.',
    icon: '📸',
  },
  {
    id: 'ar_check', image: '/Measure_02_ar_check.png', badge: '02', label: 'AI Cross-Check', color: AMBER,
    title: 'AI cross-checks its reading against the scene',
    desc: 'After making a measurement, the AI performs a second internal check — it looks for other known objects in the frame to confirm the reading is plausible. The orange atom symbol means this cross-check is running. It compares against typical stair proportions and any scale references it found.',
    detail: 'For example, if the AI measures a riser at 210mm but sees 4 risers in frame suggesting a different total rise, it will flag low confidence so you can decide whether to retry.',
    icon: '🤖',
  },
  {
    id: 'fail', image: '/Measure_03_fail.png', badge: '03', label: 'Poor Result', color: RED,
    title: 'Low confidence — try repositioning for a better photo',
    desc: 'A low confidence result means the photo did not contain enough visual information. Common causes: camera too far away (subject too small in frame), no scale reference objects visible, or the image was blurry or poorly lit.',
    detail: 'Tap ↺ Retry to take another photo. Try getting closer, improving lighting, and ensuring part of the stair structure — skirting, door frame, or even the phone edge — is visible as a known-size reference.',
    icon: '⚠️',
  },
  {
    id: 'confirmed', image: '/Measure_04_confirmed.png', badge: '04', label: 'Confirmed ✓', color: GREEN,
    title: 'Good result — blue line shows the confirmed measurement',
    desc: 'When the AI returns a confident reading, the measurement line turns blue. The two blue anchor dots mark the start and end points of the dimension measured. The dashed line shows the measured distance in millimetres.',
    detail: 'You can adjust the number using the − and + buttons if it looks slightly off (±5mm per tap, or ±1 step for step counts). Tap ✓ Confirm & Next to save and move to the next position.',
    icon: '✅',
  },
  {
    id: 'width', image: '/Measure_05_width.png', badge: '05', label: 'Stair Width', color: BLUE,
    title: 'Width measured from both edges simultaneously',
    desc: 'For stair width the AI identifies both the left and right edge in the same photo. Two blue lines run from the phone to each stringer. The arc and dashed line both show the measured width — from left stringer to right stringer across the front of the tread.',
    detail: 'Step back until both edges are visible in frame at the same time. The AI uses the riser height you already measured as a built-in scale reference to calibrate the width reading.  Width is optional — skip it if you cannot see both edges.  ',
    icon: '📐',
  },
]
