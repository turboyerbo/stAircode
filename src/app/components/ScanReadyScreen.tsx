...
export default function ScanReadyScreen({ userRole = 'diy', scanMode = 'accuracy', onSuccess, onBack }: Props) {
  ...
  const lockedRef    = useRef<number | null>(null)  // mirrors lockedMm for closure-safe reads
...
- isResidential: true if typical home stair (under 4m wide, 125-200mm risers), false if commercial/industrial, null if unsure`, 10000)
  ...
  useEffect(() => {
    ...
    async function analyse() {
      if (busyRef.current || !alive || lockedRef.current !== null) return
    ...
    // Speed mode auto-fill after 8s (use lockedRef — not stale lockedMm state)
    const autoFill = isSpeed ? setTimeout(() => {
      if (lockedRef.current === null && alive) {
        lockedRef.current = cfg.typical
...
Reply ONLY: {"phase":"locked","message":"...","estimatedMm":number,"confidence":0.0-1.0}`
  ...
  function confirmAndAdvance(mm: number) {
    ...
    setLockedMm(null)
    setResults(prev => {
      const newResults = { ...prev, [phase]: mm }
      const nextMap: Partial<Record<Phase, Phase>> = {
        riser:    'tread',
        tread:    newResults.width ? 'nosing' : 'width',
        width:    'nosing',
        handrail: 'review',
      }
      ...
      <div style={{ position:'absolute', top:0, left:0, right:0, zIndex:50,
        ...
        <div style={{ textAlign:'center' }}>
          <div style={{ fontSize:'0.65rem', fontFamily:'monospace', letterSpacing:'0.15em', color:'rgba(255,255,255,0.5)' }}>
            {(['riser','tread','width','nosing','handrail'] as Phase[]).includes(phase)
              ? `${(['riser','tread','width','nosing','handrail'] as Phase[]).indexOf(phase) + 1} / 5`
              : phase === 'detect' ? 'Detecting…' : 'Review'}
          </div>
        ...
        <div style={{ position:'absolute', bottom:0, left:0, right:0, zIndex:50,
          ...
          {lockedMm && (
            <div style={{ display:'flex', flexDirection:'column', gap:'0.5rem' }}>
              ...
              <div style={{ display:'flex', gap:'0.6rem' }}>
                <button onClick={() => { lockedRef.current = null; setLockedMm(null) }}
        ...
        <div style={{ position:'absolute', inset:0, zIndex:80,
          ...
          <div style={{ maxWidth:440, margin:'0 auto', display:'flex', flexDirection:'column', gap:'1rem' }}>
            <div style={{ textAlign:'center' }}>
              ...
              <div style={{ fontSize:'0.72rem', color:'rgba(255,255,255,0.5)', marginTop:'0.2rem' }}>Tap any number to correct it</div>
            </div>

            {([
              { key:'rise',   label:'Riser Height',   unit:'mm', numeric:true  },
              { key:'run',    label:'Tread Depth',    unit:'mm', numeric:true  },
              { key:'width',  label:'Stair Width',    unit:'mm', numeric:true  },
              { key:'nosing', label:'Nosing',         unit:'',   numeric:false },
              { key:'guard',  label:'Handrail Height', unit:'mm', numeric:true },
            ] as const).map(({ key, label, unit, numeric }) => (
              <div key={key} style={{ display:'flex', alignItems:'center', justifyContent:'space-between',
                background:'rgba(255,255,255,0.05)', borderRadius:12, padding:'0.65rem 1rem',
                border:'1px solid rgba(255,255,255,0.08)', gap:'0.75rem' }}>
                <div style={{ fontSize:'0.82rem', color:'rgba(255,255,255,0.7)', flexShrink:0 }}>{label}</div>
                {numeric ? (
                  <div style={{ display:'flex', alignItems:'center', gap:'0.3rem' }}>
                    <input
                      type="number"
                      inputMode="numeric"
                      value={reviewVals[key] != null ? String(reviewVals[key]) : ''}
                      onChange={e => {
                        const v = parseInt(e.target.value, 10)
                        if (!isNaN(v) && v > 0) setReviewVals(prev => ({ ...prev, [key]: v }))
                      }}
                      style={{
                        width:70, padding:'0.35rem 0.5rem',
                  ...
                  <div style={{ display:'flex', gap:'0.4rem' }}>
                    {(['none', 'yes'] as const).map(opt => (
                      <button key={opt}
                        onClick={() => setReviewVals(prev => ({ ...prev, nosing: opt === 'yes' ? (nosingMm || 30) : 'none' }))}
                        style={{ padding:'0.3rem 0.65rem', borderRadius:8, border:'none', cursor:'pointer',
                          fontFamily:'monospace', fontSize:'0.72rem', fontWeight:700,
                          background: (opt === 'none' ? reviewVals[key] === 'none' : reviewVals[key] !== 'none')
                            ? AMBER + '33' : 'rgba(255,255,255,0.06)',
                          color: (opt === 'none' ? reviewVals[key] === 'none' : reviewVals[key] !== 'none')
                            ? AMBER : 'rgba(255,255,255,0.5)',
                        }}>
