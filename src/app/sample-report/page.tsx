...
export default function SampleReportPage() {
  return (
    <div style={{ minHeight: '100vh', background: '#F4F7FB', fontFamily: "-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif", color: '#0D1E2E' }}>

      {/* Nav */}
      <header style={{ padding: '1rem 1.5rem', background: '#fff', borderBottom: '1px solid #E5EBF2', display: 'flex', alignItems: 'center', position: 'sticky', top: 0, zIndex: 100 }}>
        <a href="/marketing" style={{ textDecoration: 'none' }}><NavLogo height={26} /></a>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
          <a href="/marketing" style={{ fontSize: '0.78rem', color: '#5E7D9B', textDecoration: 'none' }}>← Back</a>
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '2rem 1.25rem 4rem' }}>
        ...
        {/* ── Report cover ─────────────────────────────────────────────────── */}
        <div style={{ background: 'linear-gradient(160deg,#0A1C2E 0%,#0F2E48 100%)', borderRadius: 20, overflow: 'hidden', marginBottom: '2rem', boxShadow: '0 8px 40px rgba(10,28,46,0.18)' }}>
          {/* Hazard stripe */}
          ...
          <div style={{ padding: '2rem 2.5rem 2.5rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem', marginBottom: '1.5rem' }}>
              ...
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: '0.62rem', color: 'rgba(255,255,255,0.4)', letterSpacing: '0.1em', marginBottom: '0.35rem' }}>SAMPLE DOCUMENT</div>
                <div style={{ fontSize: '0.72rem', color: '#F29337', fontWeight: 700 }}>staircode.app</div>
              </div>
            </div>
            {/* Summary pills */}
            <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
              <div style={{ background: 'rgba(39,169,107,0.15)', border: '1.5px solid rgba(39,169,107,0.35)', borderRadius: 12, padding: '0.7rem 1.25rem', textAlign: 'center' }}>
                <div style={{ fontSize: '1.8rem', fontWeight: 900, color: '#3DB88A', lineHeight: 1 }}>{PASSED}</div>
                <div style={{ fontSize: '0.6rem', color: 'rgba(255,255,255,0.5)', fontFamily: 'monospace', letterSpacing: '0.1em', marginTop: '0.2rem' }}>PASSED</div>
              </div>
              <div style={{ background: 'rgba(232,85,85,0.15)', border: '1.5px solid rgba(232,85,85,0.35)', borderRadius: 12, padding: '0.7rem 1.25rem', textAlign: 'center' }}>
                <div style={{ fontSize: '1.8rem', fontWeight: 900, color: '#E85555', lineHeight: 1 }}>{FAILED}</div>
                <div style={{ fontSize: '0.6rem', color: 'rgba(255,255,255,0.5)', fontFamily: 'monospace', letterSpacing: '0.1em', marginTop: '0.2rem' }}>FAILED</div>
              </div>
              <div style={{ background: 'rgba(242,147,55,0.12)', border: '1.5px solid rgba(242,147,55,0.3)', borderRadius: 12, padding: '0.7rem 1.25rem', textAlign: 'center' }}>
                <div style={{ fontSize: '1.8rem', fontWeight: 900, color: '#F29337', lineHeight: 1 }}>{REVIEW}</div>
                <div style={{ fontSize: '0.6rem', color: 'rgba(255,255,255,0.5)', fontFamily: 'monospace', letterSpacing: '0.1em', marginTop: '0.2rem' }}>REVIEW</div>
              </div>
              <div style={{ background: 'rgba(147,186,212,0.08)', border: '1px solid rgba(147,186,212,0.2)', borderRadius: 12, padding: '0.7rem 1.25rem', textAlign: 'center' }}>
                <div style={{ fontSize: '1.8rem', fontWeight: 900, color: '#93BAD4', lineHeight: 1 }}>{MEASUREMENTS.length}</div>
                <div style={{ fontSize: '0.6rem', color: 'rgba(255,255,255,0.5)', fontFamily: 'monospace', letterSpacing: '0.1em', marginTop: '0.2rem' }}>SCANNED</div>
              </div>
            </div>
          </div>
          <div style={{ height: 10, background: 'repeating-linear-gradient(45deg,#F29337 0,#F29337 10px,#0A1C2E 10px,#0A1C2E 20px)' }} />
        ...
        {/* ── Stair description ─────────────────────────────────────────────── */}
        <div style={{ background: '#fff', borderRadius: 16, padding: '1.5rem 2rem', marginBottom: '1.5rem', border: '1px solid #E5EBF2' }}>
          <div style={{ fontSize: '0.65rem', fontWeight: 800, letterSpacing: '0.12em', color: '#F29337', textTransform: 'uppercase', marginBottom: '0.6rem' }}>1. Stair Description</div>
          <p style={{ fontSize: '0.9rem', color: '#0D1E2E', lineHeight: 1.8, margin: '0 0 0.75rem' }}>
            The staircase under review is a concrete-structure interior stair serving a multi-unit residential building in Toronto, Ontario. The stair consists of 9 risers with a measured rise of 195mm and a tread depth (run) of 250mm, consistent with a standard Part 9 residential configuration. The stair is equipped with a wall-mounted handrail on one side measuring 900mm in height and a metal guard rail system on the opposite side.
          </p>
        ...
        <div style={{ background: '#fff', borderRadius: 16, padding: '1.5rem 2rem', marginBottom: '1.5rem', border: '1px solid #E5EBF2' }}>
          <div style={{ fontSize: '0.65rem', fontWeight: 800, letterSpacing: '0.12em', color: '#F29337', textTransform: 'uppercase', marginBottom: '1.25rem' }}>2. Measurement Photos &amp; Compliance Analysis</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.75rem' }}>
            {MEASUREMENTS.map((m) => {
              ...
              return (
                <div key={m.id} style={{ display: 'flex', flexWrap: 'wrap', gap: '1.25rem', alignItems: 'flex-start', padding: '1.25rem', background: passBg, border: `1px solid ${passBorder}`, borderRadius: 14 }}>
                  {/* Photo */}
                  <div style={{ flex: '0 0 160px', borderRadius: 12, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.1)', border: '1px solid #E5EBF2' }}>
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={m.src} alt={m.label} style={{ width: '100%', height: 120, objectFit: 'cover', objectPosition: 'center', display: 'block' }} />
                    <div style={{ background: '#0A1C2E', padding: '0.35rem 0.6rem' }}>
                      <div style={{ fontSize: '0.55rem', color: 'rgba(255,255,255,0.5)', fontFamily: 'monospace', letterSpacing: '0.08em' }}>{m.code}</div>
                    </div>
                  </div>
                  {/* Data */}
                  <div style={{ flex: '1 1 220px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.5rem', flexWrap: 'wrap' }}>
                      <span style={{ fontSize: '0.95rem', fontWeight: 800, color: '#0D1E2E' }}>{m.label}</span>
                      <span style={{ fontSize: '0.62rem', fontWeight: 800, color: passColor, background: `${passColor}18`, border: `1px solid ${passColor}40`, borderRadius: 6, padding: '0.15rem 0.6rem', fontFamily: 'monospace', letterSpacing: '0.08em' }}>{passLabel}</span>
                    </div>
                    <div style={{ fontSize: '1.6rem', fontWeight: 900, color: passColor, lineHeight: 1, marginBottom: '0.4rem', fontFamily: 'monospace' }}>
                      {m.value}<span style={{ fontSize: '0.8rem', fontWeight: 500, color: '#9BB5C8', marginLeft: '0.25rem' }}>{m.unit}</span>
                    </div>
        ...
        {/* ── Code sections ─────────────────────────────────────────────────── */}
        <div style={{ background: '#fff', borderRadius: 16, padding: '1.5rem 2rem', marginBottom: '1.5rem', border: '1px solid #E5EBF2' }}>
          <div style={{ fontSize: '0.65rem', fontWeight: 800, letterSpacing: '0.12em', color: '#F29337', textTransform: 'uppercase', marginBottom: '1rem' }}>3. Applicable Code Sections</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {[
              ...
              <div key={section} style={{ display: 'flex', gap: '0.75rem', padding: '0.75rem', background: '#F4F7FB', borderRadius: 10 }}>
                <div style={{ flexShrink: 0, fontSize: '0.72rem', fontWeight: 800, color: '#0D1E2E', fontFamily: 'monospace', minWidth: 190, paddingTop: '0.05rem' }}>{section}</div>
        ...
        {/* ── Occupancy and risk ────────────────────────────────────────────── */}
        <div style={{ background: '#fff', borderRadius: 16, padding: '1.5rem 2rem', marginBottom: '1.5rem', border: '1px solid #E5EBF2' }}>
          <div style={{ fontSize: '0.65rem', fontWeight: 800, letterSpacing: '0.12em', color: '#F29337', textTransform: 'uppercase', marginBottom: '0.75rem' }}>4. Probable Occupancy &amp; Risk Assessment</div>
          <p style={{ fontSize: '0.9rem', color: '#0D1E2E', lineHeight: 1.8, margin: '0 0 0.75rem' }}>
            Based on the dimensional profile, construction type, and Toronto location, this stair most likely serves a Part 9 multi-unit residential building. The measured rise of 195mm and run of 250mm are consistent with older residential construction common to mid-century Toronto apartment buildings.
          </p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
            <div style={{ display: 'flex', gap: '0.6rem', padding: '0.75rem', background: 'rgba(232,85,85,0.06)', border: '1px solid rgba(232,85,85,0.2)', borderRadius: 10 }}>
              <span style={{ color: '#E85555', fontWeight: 900, flexShrink: 0 }}>①</span>
            ...
            <div style={{ display: 'flex', gap: '0.6rem', padding: '0.75rem', background: 'rgba(242,147,55,0.06)', border: '1px solid rgba(242,147,55,0.2)', borderRadius: 10 }}>
              <span style={{ color: '#F29337', fontWeight: 900, flexShrink: 0 }}>②</span>
            ...
            <div style={{ display: 'flex', gap: '0.6rem', padding: '0.75rem', background: 'rgba(39,169,107,0.06)', border: '1px solid rgba(39,169,107,0.2)', borderRadius: 10 }}>
              <span style={{ color: '#27A96B', fontWeight: 900, flexShrink: 0 }}>③</span>
        ...
        {/* ── Recommendation ────────────────────────────────────────────────── */}
        <div style={{ background: '#0A1C2E', borderRadius: 16, padding: '1.5rem 2rem', marginBottom: '1.5rem', border: '1px solid rgba(147,186,212,0.15)' }}>
          <div style={{ fontSize: '0.65rem', fontWeight: 800, letterSpacing: '0.12em', color: '#F29337', textTransform: 'uppercase', marginBottom: '0.75rem' }}>5. Recommendation</div>
          <p style={{ fontSize: '0.9rem', color: 'rgba(255,255,255,0.82)', lineHeight: 1.8, margin: '0 0 0.75rem' }}>
            A formal inspection by a licensed building inspector or qualified contractor is warranted before any real estate transaction, renovation permit application, or occupancy change proceeds. The headroom deficiency is the primary item requiring correction and likely requires structural assessment of the ceiling or soffit above the stair travel path.
          </p>
        ...
        {/* ── Disclaimer ────────────────────────────────────────────────────── */}
        <div style={{ background: 'rgba(242,147,55,0.06)', border: '1.5px solid rgba(242,147,55,0.2)', borderLeft: '4px solid #F29337', borderRadius: 14, padding: '1rem 1.25rem', marginBottom: '2rem', fontSize: '0.78rem', color: '#5E7D9B', lineHeight: 1.75 }}>
          <strong style={{ color: '#C4721E' }}>Pre-Analysis Only.</strong>{' '}
        ...
        {/* ── CTA ───────────────────────────────────────────────────────────── */}
        <div style={{ background: '#fff', borderRadius: 16, padding: '2rem', textAlign: 'center', border: '1.5px solid #E5EBF2' }}>
          <div style={{ fontSize: '1.15rem', fontWeight: 900, color: '#0D1E2E', marginBottom: '0.5rem' }}>Get a report like this for your stairs</div>
